...
const ProductSlug = () => {
    ...
    const { loading, data, error } = useQuery(PRODUCT_QUERY, { variables: { slug } , fetchPolicy: 'network-only'})
    if (loading) {
        return 'Loading ...'
    }
    if (error) {
    ...
    return (
        ...
        <React.Fragment>
            <div className="Product-sell">
                ...
                <Grid item xs={6}>
                    <p className="head-name">{product.name}</p>
                    <hr></hr>
                    <div style={{ display: 'flex' }}>
                        {/* <span>1K&nbsp;</span>
                        <span style={{ color: '#9E9E9E' }}>&nbsp;Sold |&nbsp;</span> */}
                        <div className="tag-product">{product.type}</div>
                    </div>
                ...
                <Grid item xs={6} style={{ marginLeft: '1rem', display: 'flex', flexDirection: 'column' }}>
                    <img src={product.imageUrl} className="img-product" />
                </Grid>
            </div>
                ...
                <div className="Product-Speci">
                    <div className="Detail-box">
                        ...
                        <Grid item xs={8} style={{ color: '#202C39' }}>
                            ...
                            <span>{product.quantity}</span><br></br>
                        </Grid>
                    </div>
                </div>
